...
(function(){
  ...
  // Create subtle vintage background texture
  ...
  backgroundContainer.style.cssText = `
    ...
    z-index: -2;
    background: 
      radial-gradient(ellipse at 30% 20%, rgba(184,148,95,.06), transparent 50%),
      radial-gradient(ellipse at 70% 80%, rgba(212,196,168,.04), transparent 50%),
      linear-gradient(135deg, #f5f1e8 0%, #faf8f3 50%, #f0ebe0 100%);
    opacity: 0.6;
  `;
  document.body.appendChild(backgroundContainer);
  
  // Add subtle vintage particles (fewer, more subtle)
  for (let i = 0; i < 20; i++) {
    ...
    particle.style.cssText = `
      ...
      background: ${Math.random() > 0.5 ? 'rgba(184,148,95,0.3)' : 'rgba(212,196,168,0.25)'};
      border-radius: 50%;
  ...
  centralGlow.style.cssText = `
    ...
    z-index: -1;
    pointer-events: none;
    background: radial-gradient(circle, rgba(184,148,95,0.08) 0%, rgba(212,196,168,0.04) 50%, transparent 70%);
    border-radius: 50%;
  ...
  // Subtle vintage particles (reduced count and opacity)
  const COUNT = 400;
  ...
  const pMat = new THREE.PointsMaterial({ color: 0xb8945f, size: 0.008, transparent:true, opacity:0.3 });
  const points = new THREE.Points(pGeo, pMat);
  scene.add(points);

  // Subtle vintage lighting
  scene.add(new THREE.AmbientLight(0xfaf8f3, 0.6));
  
  // Warm directional light
  const dl = new THREE.DirectionalLight(0xb8945f, 0.8);
  ...
  // Subtle point light
  const pointLight = new THREE.PointLight(0xb8945f, 0.5, 10);
  ...
  function animate(){
    ...
    // Animate central glow (subtle)
    const glowScale = 1 + Math.sin(t * 0.3) * 0.1;
